...
oop四步走转化公式主要目的说的是封装，和封装关系更大。这里顺便补充继承和多态。

男孩和女孩的大部分属性和方法都是一样的，但是也有不同，就要继承重写，例如小便姿势。

## 7.1 oop来实现继承 多态，非常丝滑方便
```python
class Person:
    def __init__(self, name, height, weight):
        self.name = name
...
### 调用不同模块的函数
```python
boy.pee("小明", 100.1, 55, 5)

gril.pee("小红", 95.1, 45, 5)
```

### 小结：
- 使用极端面向过程来模拟继承，封装极其痛苦，需要全量复制所有代码，然后去扣字母修改部分代码，形成新的py文件。 
- 这是亲自见过的一种写法，项目种几十个文件，一眼扫去，每个文件的代码结构和长度惊人相似，只有少部分几行不同，就是因为不使用 面向对象 继承造成的。
